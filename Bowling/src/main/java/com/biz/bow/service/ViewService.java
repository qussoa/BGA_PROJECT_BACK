	...
	public PlayerVO showBowGame(List<FrameService> frames, PlayerService player) {
		...
		for (int i = 0; i < frames.size(); i++) {
			...
			if (frame.getState() != "NOMAL") {
				if(i < 8) {
					resultScore.calScoreWithNextFrame(frame.getState(), frames.get(i),frames.get(i + 1), frames.get(i + 2));
				} else if (i == 8) {
					resultScore.calNineScoreWithNextFrame(frame.getState(), frames.get(i+1));
				}
			}
		...
		playerVO.setTotalScore(resultScore.getTotalScore());
		System.out.println("총점 :"+playerVO.getTotalScore());
